...
const InventoryRow = memo(function InventoryRow({ row, selected, onToggle, canManage, deletePending, onEdit, onDelete }) {
  ...
  const supplierName = row.supplier_name || row.product?.supplier_name || '—';
    ...
    <tr className={selected ? 'inv-row--selected' : ''}>
      ...
      <td>
        <div className="inv-qty-cell">
          ...
          {showWarning ? <AlertTriangle size={14} className="inv-qty-warning" /> : null}
        </div>
      </td>
      ...
      <td>
        <div className="catalog-action-group">
          <button type="button" className="catalog-icon-btn" onClick={handleEdit} title="Edit" disabled={!canManage}>
            <Edit size={15} />
          </button>
          <button type="button" className="catalog-icon-btn" onClick={handleRestock} title="Restock" disabled={!canManage}>
            <RefreshCw size={15} />
          </button>
          <button type="button" className="catalog-icon-btn" onClick={handleAdjust} title="Adjust" disabled={!canManage}>
            <SlidersHorizontal size={15} />
          </button>
          <button type="button" className="catalog-icon-btn danger" onClick={handleDelete} title="Delete" disabled={!canManage || deletePending}>
            <Trash2 size={15} />
          </button>
        </div>
      </td>
      ...
      <div className="table-wrap">
        <table className="data-table">
          ...
          <tbody>
            {!storeId ? (
              <tr>
                <td colSpan="9" className="catalog-empty-cell">Select a store first.</td>
              </tr>
              ...
              <tr>
                <td colSpan="9" className="catalog-empty-cell">No inventory rows found.</td>
              </tr>
            )}
          </tbody>
        </table>
