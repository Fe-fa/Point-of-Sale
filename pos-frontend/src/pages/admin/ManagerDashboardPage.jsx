...
const [actionNotice, setActionNotice] = useState(null);
  ...
  const [cashierCloseTarget, setCashierCloseTarget] = useState(null); // { userId, name }
  ...
  const handleLoadAllCashiers = useCallback(async () => {
    if (!storeId) return;
    ...
    try {
      const response = await cashierShiftService.allCashiers(Number(storeId));
  ...
  // ── Open per-cashier close shift modal (manager override) ──
  const handleOpenCashierClose = useCallback(async (cashierUserId, cashierName) => {
    if (!storeId) return;

    setCashierCloseError('');
    setCashierCloseTarget({ userId: cashierUserId, name: cashierName });
  ...
  // ── Confirm per-cashier close shift (manager override) ──
  const handleCashierCloseConfirm = useCallback(async ({ countedCash, variance, closeNote }) => {
    if (!storeId || !cashierCloseTarget?.userId) return;
    ...
    try {
      ...
      setActionNotice({
        type: 'success',
        message: `Shift closed successfully for ${cashierCloseTarget.name}. Variance saved and will carry forward to next shift.`,
      });
      await refresh();
    } catch (err) {
  ...
  }, [storeId, cashierCloseTarget, refresh]);
  ...
  const pendingApprovalCount = toNumber(todayData.pending_orders);
  const openCashierShiftCount = toNumber(todayData.open_cashier_shifts);
  const shiftClosureBlocked = unresolvedVoids > 0 || pendingApprovalCount > 0;
  const expectedCash = toNumber(todayData.expected_drawer_cash ?? todayData.gross_sales);

  // ── Real active shift from daily cashier summary ──
  const activeShift = useMemo(
    () => getPrimaryActiveShift(dailyCashierSummary),
    [dailyCashierSummary]
  );

  // ── Live shift timer state ──
  const [shiftTimerNow, setShiftTimerNow] = useState(() => Date.now());

  useEffect(() => {
    const timer = window.setInterval(() => setShiftTimerNow(Date.now()), 30000);
    return () => window.clearInterval(timer);
  }, []);

  const shiftTimerLabel = activeShift?.opened_at
    ? formatShiftElapsed(activeShift.opened_at, shiftTimerNow)
    : 'No open shift';
  const shiftTimerMeta = activeShift
    ? `${activeShift.cashier_name || 'Cashier'} · ${activeShift.store_name || 'Store'}`
    : `${openCashierShiftCount} active shift${openCashierShiftCount === 1 ? '' : 's'}`;
...
const handleFinalizeShift = useCallback(async ({ countedCash, variance }) => {
  ...
  // ── Block if open cashier shifts exist ──
  if (openCashierShiftCount > 0) {
    setShiftClosureError(
      `Close each active cashier shift one by one from the cashier report before running the final store Z-Report. ${openCashierShiftCount} open shift(s) remaining.`
    );
    return;
  }
  ...
  if (shiftClosureBlocked) {
    ...
    setShiftClosureError(
      `Clear unresolved cashier items before finalizing: ${blockers.join(' and ')}.`
    );
    return;
  }
...
}, [refresh, shiftClosureBlocked, storeId, unresolvedVoids, pendingApprovalCount, openCashierShiftCount, todayData.gross_sales]);
...
{/* All Cashiers Daily Sales Modal */}
...
{/* Per-cashier close shift modal (manager override) */}
{showCashierCloseModal && (
  ...
  />
      ...
      <div className="mg-hero-card">
        ...
        <div className="mg-hero-stats">
          <div className="mg-hero-stat">
            ...
            <strong className="mg-hero-stat__value">{shiftTimerLabel}</strong>
            <small>{shiftTimerMeta}</small>
          </div>
            ...
            <div className="mg-hero-stat__split">
              <div>
                ...
                <button
                  type="button"
                  className="mg-link"
                  onClick={() => document.getElementById('voids-panel')?.scrollIntoView({ behavior: 'smooth', block: 'center' })}
                >
                  View void status
                </button>
              </div>
              <div>
                ...
                <button
                  type="button"
                  className="mg-link"
                  onClick={() => document.getElementById('approvals')?.scrollIntoView({ behavior: 'smooth', block: 'center' })}
                >
                  Pending approvals
                </button>
              </div>
          ...
          <div className="mg-hero-stat" id="shift-closure-panel">
            ...
            <small className={shiftClosureBlocked || openCashierShiftCount > 0 ? 'mg-text-warn' : 'mg-text-success'}>
              {openCashierShiftCount > 0
                ? `${openCashierShiftCount} open cashier shift${openCashierShiftCount === 1 ? '' : 's'} · close individually first`
                : shiftClosureBlocked
            ...
            <button
              ...
              onClick={openShiftClosure}
            >
              {openCashierShiftCount > 0 ? 'View cashier reconciliations' : 'Open drawer reconciliation'}
            </button>
          </div>
        </div>
      </div>
      ...
      {(shiftClosureBlocked || openCashierShiftCount > 0) ? (
        <div className="mg-banner mg-banner--info" id="voids-panel">
          ...
          <div>
            ...
            {openCashierShiftCount > 0 ? (
              <p>
                {openCashierShiftCount} open cashier shift{openCashierShiftCount === 1 ? '' : 's'} must be closed individually before the final store Z-Report can be run.
              </p>
            ) : (
              <p>
                Voids must be cleared before the Drawer Reconciliation can finalize a shift closure.
      ...
      <div style={{ marginBottom: 24 }}>
        {/* <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 12 }}>
          <DailySalesSummaryCard
            ...
            currentCurrency={currentCurrency}
            className="mg-card"
            emptyText="No cashier shift has been opened for this store today."
            showCarryForward
          />
        </div> */}
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
          <button
            type="button"
            className="mg-qa-btn mg-qa-btn--primary"
            onClick={handleLoadAllCashiers}
            disabled={allCashiersLoading}
          >
            <Users size={14} /> View All Cashiers Report
          </button>
          {dailyCashierSummary.filter(r => r.status === 'open').map((row) => (
            <button
              key={`${row.cashier_user_id}-${row.store_id}`}
              type="button"
              className="mg-qa-btn mg-qa-btn--accent"
              onClick={() => handleOpenCashierClose(row.cashier_user_id, row.cashier_name)}
              title={`Close shift for ${row.cashier_name}`}
            >
              <Lock size={14} /> Close {row.cashier_name}'s Shift
            </button>
          ))}
        </div>
      </div>
