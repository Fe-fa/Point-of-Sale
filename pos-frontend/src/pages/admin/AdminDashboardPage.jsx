...
import DailySalesSummaryCard from '../../components/reports/DailySalesSummaryCard';
import AllCashiersDailySalesModal from '../../components/modals/AllCashiersDailySalesModal';
...
const formatShiftElapsed = (openedAt, nowValue = Date.now()) => {
  if (!openedAt) return 'No open shift';
  const opened = new Date(openedAt);
  if (Number.isNaN(opened.getTime())) return 'No open shift';

  const diffMs = Math.max(nowValue - opened.getTime(), 0);
...
const getPrimaryActiveShift = (rows = []) => {
  const activeRows = (Array.isArray(rows) ? rows : [])
    .filter((row) => row?.status === 'open' && row?.opened_at)
    .sort((a, b) => new Date(a.opened_at).getTime() - new Date(b.opened_at).getTime());

  return activeRows[0] || null;
};

const buildLicenseStatus = (store) => {
  const status = String(store?.status || '').toLowerCase();
  const orders = toNumber(store?.orders);
  const lowStock = toNumber(store?.lowStock);
  const outstanding = toNumber(store?.outstanding);

  if (status === 'inactive') {
    return { status: 'inactive', label: 'Inactive tenant', tone: '#92a0ae' };
  }

  if (outstanding > 0 || lowStock > 0) {
    return {
      status: 'warning',
      label: `${outstanding} open balance${outstanding === 1 ? '' : 's'} · ${lowStock} low-stock alert${lowStock === 1 ? '' : 's'}`,
      tone: '#f08c4a',
    };
  }

  if (orders === 0) {
    return { status: 'urgent', label: 'No recent transaction activity', tone: '#d6336c' };
  }

  return { status: 'ok', label: `${orders} orders processed`, tone: '#18a36a' };
};
      ...
      >
        ...
        <div className="modal-body mg-zr-body">
          ...
          {report.products_sold?.length > 0 && (
            <div className="mg-zr-section">
              <p className="mg-zr-section-title">Products Sold</p>

              {report.products_sold.map((item, i) => (
                <div key={i} className="mg-zr-row">
                  <span>
                    {item.product_name} <small>({item.qty} pcs)</small>
                  </span>
                  <strong>{cur} {toNumber(item.amount).toFixed(2)}</strong>
                </div>
              ))}
...
/* =====================================================================
   DB BACKUPS MODAL
   ===================================================================== */
const DbBackupsModal = memo(function DbBackupsModal({
  onClose,
  summary,
  dailyCashierSummary,
  auditEvents,
  ...
  const rows = useMemo(() => {
    ...
    const visibleAuditEvents = Array.isArray(auditEvents) ? auditEvents : [];
    const visibleShiftRows = Array.isArray(dailyCashierSummary) ? dailyCashierSummary : [];
    const activeShiftCount = visibleShiftRows.filter((row) => row?.status === 'open').length;
    const closedShiftCount = visibleShiftRows.filter((row) => row?.status === 'closed').length;

    return [
      {
        name: 'Audit log trail',
        detail: visibleAuditEvents.length
          ? `${visibleAuditEvents.length} recent log entr${visibleAuditEvents.length === 1 ? 'y' : 'ies'} visible in the dashboard`
          : 'No recent audit log rows returned for this scope',
        status: visibleAuditEvents.length ? 'success' : 'warning',
      },
      {
        name: 'Cashier shift snapshots',
        detail: `${activeShiftCount} open shift${activeShiftCount === 1 ? '' : 's'} · ${closedShiftCount} closed shift${closedShiftCount === 1 ? '' : 's'} today`,
        status: activeShiftCount > 0 ? 'warning' : 'success',
      },
      {
        name: 'Sales ledger recovery points',
        detail: `${toNumber(stats.total_orders)} orders · ${currency(today.outstanding, currentCurrency)} outstanding still traceable`,
        status: toNumber(stats.total_orders) > 0 ? 'success' : 'warning',
      },
      {
        name: 'Master data persistence',
        detail: `${toNumber(stats.products)} products · ${toNumber(stats.customers)} customers · ${toNumber(inventory.total_rows)} inventory rows`,
        status: toNumber(stats.products) > 0 || toNumber(stats.customers) > 0 ? 'success' : 'warning',
      },
    ];
  }, [summary, dailyCashierSummary, auditEvents, currentCurrency]);

  const lastAuditAt = auditEvents?.[0]?.created_at || auditEvents?.[0]?.timestamp || null;
      ...
      <div className="modal-box" style={{ maxWidth: 620 }} onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            ...
            <p className="modal-sub">Live persistence and recovery signals from the current database scope</p>
          </div>
        ...
        <div className="modal-body">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {rows.map((row) => (
              <div key={row.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 8, background: 'var(--sa-surface-soft, #f7fafc)', border: '1px solid var(--sa-border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <Database size={15} color={row.status === 'warning' ? '#f08c4a' : '#18a36a'} />
                  <div>
                    <strong style={{ fontSize: '0.85rem' }}>{row.name}</strong>
                    <p style={{ fontSize: '0.75rem', color: 'var(--sa-text-soft)', margin: 0 }}>{row.detail}</p>
                  </div>
                </div>
                <span style={{
                  fontSize: '0.72rem', padding: '2px 8px', borderRadius: 999, fontWeight: 600,
                  background: row.status === 'warning' ? '#fff3e0' : '#e8f7f0',
                  color: row.status === 'warning' ? '#f08c4a' : '#18a36a'
                }}>
                  {row.status === 'warning' ? 'Needs review' : 'Recoverable'}
                </span>
              </div>
            ))}
          </div>

          <div style={{ marginTop: 14, padding: '12px 14px', borderRadius: 8, background: '#eef6ff', fontSize: '0.8rem', color: '#0e84c3', border: '1px solid #cfe2ff' }}>
            {lastAuditAt
              ? `Latest visible audit activity: ${new Date(lastAuditAt).toLocaleString('en-KE')}.`
              : 'Audit history is currently empty for this scope.'} Recovery evidence on this screen comes from live database records, cashier shift snapshots, inventory rows, and audit logs.
          </div>

          <div className="modal-footer" style={{ marginTop: 16 }}>
            <button type="button" className="ghost-button" onClick={onClose}>Close</button>
            <button type="button" className="primary-button" onClick={onJumpToLogs}>Open Audit Logs</button>
          </div>
        </div>
      </div>
    </div>
...
/* =====================================================================
   LICENSE RENEWALS MODAL — uses real storePerformance data
   ===================================================================== */
const LicenseRenewalsModal = memo(function LicenseRenewalsModal({ onClose, storePerformance, currentCurrency }) {
  const licenses = useMemo(() => {
    return (Array.isArray(storePerformance) ? storePerformance : []).map((store, index) => {
      const derived = buildLicenseStatus(store);
      return {
        id: `${store?.store_id || index}`,
        tenant: store?.store_name || `Store ${index + 1}`,
        plan: store?.tier || 'Standard',
        status: derived.status,
        statusLabel: derived.label,
        tone: derived.tone,
        revenue: toNumber(store?.revenue),
        orders: toNumber(store?.orders),
        outstanding: toNumber(store?.outstanding),
      };
    });
  }, [storePerformance]);
      ...
      <div className="modal-box" style={{ maxWidth: 620 }} onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div>
            ...
            <p className="modal-sub">Live tenant health and license-review priority from current store activity</p>
          </div>
          <button type="button" className="icon-btn" onClick={onClose} aria-label="Close"><X size={18} /></button>
        </div>
        <div className="modal-body">
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            {[
              { label: 'Urgent review', count: counts.urgent, color: '#d6336c', bg: '#fff0f0', border: '#f8c8c8' },
              { label: 'Operational watch', count: counts.warning, color: '#f08c4a', bg: '#fff8ee', border: '#f5deb3' },
              { label: 'Healthy', count: counts.ok, color: '#18a36a', bg: '#e8f7f0', border: '#b2dfcf' },
              { label: 'Inactive', count: counts.inactive, color: '#92a0ae', bg: '#f4f7f9', border: '#e3eaee' },
            ].map((tile) => (
              <div key={tile.label} style={{ flex: 1, minWidth: 96, padding: '10px 12px', borderRadius: 8, background: tile.bg, border: `1px solid ${tile.border}`, textAlign: 'center' }}>
                <strong style={{ color: tile.color, fontSize: '1.2rem', display: 'block' }}>{tile.count}</strong>
                <p style={{ margin: 0, fontSize: '0.7rem', color: tile.color }}>{tile.label}</p>
              </div>
            ))}
          </div>
          ...
          {licenses.length ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 320, overflowY: 'auto' }}>
              {licenses.map((license) => (
                <div key={license.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 8, background: 'var(--sa-surface-soft, #f7fafc)', border: '1px solid var(--sa-border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Key size={14} color={license.tone} />
                    <div>
                      <strong style={{ fontSize: '0.84rem' }}>{license.tenant}</strong>
                      <p style={{ margin: 0, fontSize: '0.73rem', color: 'var(--sa-text-soft)' }}>{license.plan} · {license.statusLabel}</p>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <strong style={{ display: 'block', fontSize: '0.78rem', color: license.tone }}>{currency(license.revenue, currentCurrency)}</strong>
                    <span style={{ fontSize: '0.72rem', color: 'var(--sa-text-soft)' }}>{license.orders} orders · {license.outstanding} open balance{license.outstanding === 1 ? '' : 's'}</span>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div style={{ padding: 20, textAlign: 'center', color: 'var(--sa-text-soft)', fontStyle: 'italic' }}>No store data available yet.</div>
          )}
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={tile.label} style={{
                flex: 1, minWidth: 100, padding: '10px 14px', borderRadius: 8, textAlign: 'center',
...
const GlobalOpsBanner = memo(function GlobalOpsBanner({
  ...
  const shiftLabel = activeShift?.opened_at
    ? formatShiftElapsed(activeShift.opened_at, timerNow)
    : 'No open shift';
  const shiftMetaLabel = activeShift
    ? `${activeShift.cashier_name || 'Cashier'} · ${activeShift.store_name || 'Store'}`
    : `${openShiftCount} active shift${openShiftCount === 1 ? '' : 's'} in scope`;
    ...
    <div className="sa-global-ops">
      ...
      <div className="sa-global-ops__kpis">
        <div className="sa-ops-kpi sa-ops-kpi--blue">
          ...
          <div className="sa-ops-kpi__sub">{shiftMetaLabel}</div>
        </div>
        ...
        <div className="sa-ops-kpi sa-ops-kpi--pink">
          ...
          <div className="sa-ops-kpi__row">
            <div>
              <div className="sa-ops-kpi__value">{voids}</div>
              <div className="sa-ops-kpi__sub">
                <button className="sa-ops-link" type="button" onClick={onOpenVoids}>Open queue</button>
              </div>
            </div>
            <div>
              <div className="sa-ops-kpi__value">{approvalQueueCount}</div>
              <div className="sa-ops-kpi__sub">
                <button className="sa-ops-link" type="button" onClick={onOpenApprovals}>Open logs</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="sa-ops-kpi sa-ops-kpi--amber">
          ...
          <div className="sa-ops-kpi__sub" style={{ fontSize: '0.72rem', opacity: 0.85 }}>
            {shiftClosureBlocked
              ? 'Resolve open cashier items before final store closure'
              : openShiftCount > 0
                ? 'Close active cashiers one by one'
                : 'Ready for final Z-report'}
          </div>
          <div className="sa-ops-kpi__sub">
            <button
            ...
            >
              {openShiftCount > 0 ? 'View cashier reconciliations' : 'Open reconciliation'}
            </button>
          </div>
        </div>
...
export default function SuperAdminDashboardPage() {
  ...
  const dailyCashierSummary = summary.daily_cashier_summary || [];
  ...
  // ── All Cashiers Daily Sales modal state ─────────────────────────────
  const [showAllCashiersModal, setShowAllCashiersModal] = useState(false);
  const [allCashiersData, setAllCashiersData] = useState(null);
  const [allCashiersLoading, setAllCashiersLoading] = useState(false);

  // ── Per-cashier close shift modal state ──────────────────────────────
  const [showCashierCloseModal, setShowCashierCloseModal] = useState(false);
  const [cashierCloseLoading, setCashierCloseLoading] = useState(false);
  const [cashierCloseError, setCashierCloseError] = useState('');
  const [cashierCloseReport, setCashierCloseReport] = useState(null);
  const [cashierCloseReportLoading, setCashierCloseReportLoading] = useState(false);
  const [cashierCloseTarget, setCashierCloseTarget] = useState(null);
  ...
  const pendingApprovalCount = toNumber(todayData?.pending_orders ?? dailyCashierSummary.reduce((sum, row) => sum + toNumber(row?.pending_orders), 0));
  const shiftClosureBlocked = unresolvedVoids > 0 || pendingApprovalCount > 0;
  const activeShift = useMemo(() => summary.active_shift_focus || getPrimaryActiveShift(dailyCashierSummary), [summary.active_shift_focus, dailyCashierSummary]);
  const openShiftCount = dailyCashierSummary.filter((row) => row?.status === 'open').length;
  const reconciliationStoreId = activeShift?.store_id || selectedStoreId || storePerformance[0]?.store_id || dailyCashierSummary[0]?.store_id || null;
  const expectedCash = toNumber(todayData?.expected_drawer_cash ?? activeShift?.expected_cash ?? todayData?.collected);
  ...
  const scrollToVoids = useCallback(() => {
    document.getElementById('voids-approvals-section')
      ?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, []);

  const scrollToAuditLogs = useCallback(() => {
    document.getElementById('audit-logs-section')
      ?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, []);

  const scrollToCashierReconciliation = useCallback(() => {
    document.getElementById('cashier-reconciliation-section')
      ?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, []);
  ...
  const handleFinalizeShift = useCallback(async ({ countedCash, variance }) => {
    ...
    if (shiftClosureBlocked) {
      const blockers = [];
      if (unresolvedVoids > 0) blockers.push(`${unresolvedVoids} void(s)`);
      if (pendingApprovalCount > 0) blockers.push(`${pendingApprovalCount} pending approval/draft item(s)`);
      setShiftClosureError(`Clear unresolved cashier items before finalizing: ${blockers.join(' and ')}.`);
      return;
    }

    const storeId = reconciliationStoreId;
    if (!storeId) {
  ...
  }, [shiftClosureBlocked, unresolvedVoids, pendingApprovalCount, expectedCash, reconciliationStoreId, refresh]);
  ...
  // ── Load all cashiers daily sales for the modal ──
  const handleLoadAllCashiers = useCallback(async (targetStoreId) => {
    if (!targetStoreId) return;

    setAllCashiersLoading(true);
    setShowAllCashiersModal(true);
    setAllCashiersData(null);

    try {
      const response = await cashierShiftService.allCashiers(Number(targetStoreId));
      setAllCashiersData(response?.data || response || null);
    } catch (err) {
      setActionNotice({
        type: 'warning',
        message: err?.response?.data?.message || err?.message || 'Unable to load all cashiers report.',
      });
      setShowAllCashiersModal(false);
    } finally {
      setAllCashiersLoading(false);
    }
  }, []);

  // ── Open per-cashier close shift modal (admin override) ──
  const handleOpenCashierClose = useCallback(async (cashierUserId, cashierName, targetStoreId) => {
    if (!targetStoreId) return;

    setCashierCloseError('');
    setCashierCloseTarget({ userId: cashierUserId, name: cashierName, storeId: targetStoreId });
    setShowCashierCloseModal(true);
    setCashierCloseReportLoading(true);
    setCashierCloseReport(null);

    try {
      const response = await cashierShiftService.dailySales(Number(targetStoreId), {
        cashier_user_id: cashierUserId,
      });
      setCashierCloseReport(response?.data || response || null);
    } catch (err) {
      setCashierCloseError(err?.response?.data?.message || err?.message || 'Unable to load shift data.');
    } finally {
      setCashierCloseReportLoading(false);
    }
  }, []);

  // ── Confirm per-cashier close shift (admin override) ──
  const handleCashierCloseConfirm = useCallback(async ({ countedCash, variance, closeNote }) => {
    if (!cashierCloseTarget?.storeId || !cashierCloseTarget?.userId) return;

    setCashierCloseLoading(true);
    setCashierCloseError('');

    try {
      await cashierShiftService.close({
        store_id: Number(cashierCloseTarget.storeId),
        cashier_user_id: cashierCloseTarget.userId,
        counted_cash: countedCash,
        close_note: closeNote,
      });

      setShowCashierCloseModal(false);
      setActionNotice({
        type: 'success',
        message: `Shift closed successfully for ${cashierCloseTarget.name}. Variance saved and carried forward safely.`,
      });
      await refresh();
      if (showAllCashiersModal) {
        await handleLoadAllCashiers(cashierCloseTarget.storeId);
      }
    } catch (err) {
      setCashierCloseError(err?.response?.data?.message || err?.message || 'Unable to close shift.');
    } finally {
      setCashierCloseLoading(false);
    }
  }, [cashierCloseTarget, refresh, showAllCashiersModal, handleLoadAllCashiers]);

  const handleDrawerReconciliation = useCallback(() => {
    setShiftClosureError('');
    if (openShiftCount > 0 && reconciliationStoreId) {
      setActionNotice({
        type: 'info',
        message: 'Close each active cashier shift one by one from the cashier report before running the final Z-report.',
      });
      scrollToCashierReconciliation();
      void handleLoadAllCashiers(reconciliationStoreId);
      return;
    }

    openShiftClosure();
  }, [openShiftCount, reconciliationStoreId, scrollToCashierReconciliation, handleLoadAllCashiers, openShiftClosure]);

  return (
    <section className="sa-page">
      {/* ── ERROR / ACTION BANNERS ───────────────────── */}
      {error ? (
      ...
      <GlobalOpsBanner
        ...
        onShiftClosure={handleDrawerReconciliation}
        ...
        approvalQueueCount={pendingApprovalCount}
        onOpenVoids={scrollToVoids}
        onOpenApprovals={scrollToAuditLogs}
      />


{/* ── TOP METRICS ──────────────────────────────── */}
<div className="sa-metric-grid">
  {/* Financial Metrics -> Sun Orange Palette */}
  <MetricCard 
    icon={Wallet} 
  ...
  {/* Platform Metrics -> Deep Teal-Blue Palette */}
  <MetricCard 
    icon={Building2} 
  ...
  {/* Sign-ups / Focus States -> Deep Teal-Blue Palette */}
  <MetricCard 
    icon={UserPlus} 
  ...
  {/* Cancellations / Outflows -> Accent Toned Alert Slate */}
  <MetricCard 
    icon={AlertTriangle} 
    label="Churn rate" 
  ...
  />
      ...
      <div id="cashier-reconciliation-section" style={{ margin: '24px 0' }}>
        {/* <DailySalesSummaryCard
          title="Daily cashier sales summary"
          rows={dailyCashierSummary}
          currentCurrency={currentCurrency}
          className="sa-card"
          emptyText="No cashier shift has been opened in the selected scope today."
          showCarryForward
        /> */}
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 12 }}>
          {storePerformance.length > 0 && (
            <button
              type="button"
              className="ghost-button"
              onClick={() => handleLoadAllCashiers(storePerformance[0]?.store_id)}
              disabled={allCashiersLoading}
              style={{ minHeight: 36 }}
            >
              <Users size={15} /> View All Cashiers Report
            </button>
          )}
          {dailyCashierSummary.filter(r => r.status === 'open').slice(0, 5).map((row) => (
            <button
              key={`${row.cashier_user_id}-${row.store_id}`}
              type="button"
              className="ghost-button"
              onClick={() => handleOpenCashierClose(row.cashier_user_id, row.cashier_name, row.store_id)}
              disabled={cashierCloseLoading || cashierCloseReportLoading}
              style={{ minHeight: 36, borderColor: 'rgba(207, 58, 58, 0.3)', color: '#cf3a3a' }}
            >
              <Lock size={15} /> Close {row.cashier_name}'s Shift
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="sa-grid sa-grid--3">
        ...
        <OperationalChecklist
          ...
          onAction={(item) => {
            ...
            else if (item.type === 'zreport') handleDrawerReconciliation();
            else if (item.type === 'voids') scrollToVoids();
          }}
        />
      </div>
      ...
      <div className="sa-grid sa-grid--2">
        <article className="sa-card" id="voids-approvals-section">
      ...
      {/* ── TOP TENANTS + AUDIT ──────────────────────── */}
      <div className="sa-grid sa-grid--2">
        <article className="sa-card" id="audit-logs-section">
          <div className="sa-card__header">
            <div>
      ...
      <AllCashiersDailySalesModal
        isOpen={showAllCashiersModal}
        loading={allCashiersLoading}
        data={allCashiersData}
        currentCurrency={currentCurrency}
        onClose={() => setShowAllCashiersModal(false)}
        storePerformance={storePerformance}
        selectedStoreId={selectedStoreId || reconciliationStoreId}
        onStoreChange={(storeId) => handleLoadAllCashiers(storeId)}
      />

      {/* Per-cashier close shift modal (admin override) */}
      {showCashierCloseModal && (
        <CashierCloseShiftModal
          isOpen={showCashierCloseModal}
          loading={cashierCloseLoading || cashierCloseReportLoading}
          error={cashierCloseError}
          shiftData={cashierCloseReport}
          currentCurrency={currentCurrency}
          isManagerOverride={true}
          onClose={() => {
            if (cashierCloseLoading) return;
            setShowCashierCloseModal(false);
            setCashierCloseError('');
          }}
          onConfirm={handleCashierCloseConfirm}
        />
      )}
      {dbBackupsOpen && (
        <DbBackupsModal
          ...
          summary={summary}
          dailyCashierSummary={dailyCashierSummary}
          auditEvents={auditEvents}
          currentCurrency={currentCurrency}
          onJumpToLogs={() => {
            setDbBackupsOpen(false);
            scrollToAuditLogs();
          }}
        />
      )}
