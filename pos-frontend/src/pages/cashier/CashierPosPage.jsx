...
import { useCashierShift } from '../../contexts/CashierShiftContext';
...
export default function CashierPosPage() {
  ...
  const handlePayNow = useCallback(
    (product) => {
      if (!ensureCashShiftOpen()) return;
      handleAddProduct(product);
      window.requestAnimationFrame(() => setShowPaymentModal(true));
    },
    [ensureCashShiftOpen, handleAddProduct]
  );
...
try {
  if (!hasOpenCashShift) {
    throw new Error('Open your cash shift and record the opening balance before processing payment.');
  }

  let paidBilling = null;

  // ── NEW: M-Pesa STK already recorded the payment server-side. ──
  // Skip chargeExisting/charge entirely — just re-fetch the paid billing.
  if (paymentDetails.alreadyRecorded && current?.billing_id) {
    const paidResponse = await billingService.show(current.billing_id);
    paidBilling = paidResponse?.data || paidResponse;
  }
  else {
    const hasSplitAllocations = Array.isArray(paymentDetails.paymentAllocations)
      && paymentDetails.paymentAllocations.length > 0;

    const paymentPayload = hasSplitAllocations
      ? {
        payment_allocations: paymentDetails.paymentAllocations,
    ...
    // 3. Pure local cart -> atomic one-shot checkout
      ...
      const payload = {
        ...
        payment: hasSplitAllocations
          ? {
            allocations: paymentDetails.paymentAllocations,
            points_redeemed: paymentDetails.pointsToRedeem,
          }
          : {
...
<PaymentModal
  ...
  onEnsureBilling={async () => {
    const current = billingRef.current;
    if (current?.billing_id) return current;
    const ensured = await promoteLocalCartToServerDraft();
    if (ensured) mergeDraftPreview(ensured);
    return ensured;
  }}
